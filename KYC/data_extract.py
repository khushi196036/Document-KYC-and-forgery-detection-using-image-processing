...
def extract_data_and_image(image_path):
    ...
    if name and aadhaar:
        ...
        if len(name_parts) >= 3:  # Check if there are at least 3 parts after splitting
            extracted_img_name = name_parts[0] + "_" + aadhaar.split(' ')[2]
        else:
            extracted_img_name = "default_name"  # Provide a default name if splitting fails
    else:
        extracted_img_name = "default_name"  # Provide a default name if 'name' or 'aadhaar' is None
